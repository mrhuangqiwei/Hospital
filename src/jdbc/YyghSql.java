...
public class YyghSql {
	...
	{ // 挂号员编码
		...
		if (ConvertTime.datesdiferrent(yyghrq)) {
			...
			ghxh = Yyghdrgh(yyghrq, brxm, brxb, brnl, sfzh, jtzz, sj, ysbm,
					yyks, yydjrq, yyyxrq, ylkh, yyys, ghfsbm, ghybm, ghyks,
					ywckbm);
			yyghBean yyghBean = new yyghBean(ghxh, yyid);
			String json = JSON.toJSONString(yyghBean,
			...
			try {
				...
				if (JdbcUtilSql.getuserghbrxx(sfzh, ylkh, stmt) == null) {
					...
					if (k >= 1) {
						...
						String insetghbString = Insertyygh(yyid, ywckbm, ghybm,
								yyghrq, brxm, brxb, "", brnl, "1", sfzh, jtzz,
								sj, ysbm, yyks, yydjrq, yyyxrq, ghyks, ylkh);
						...
						if (j >= 1) {
							...
							String brzcString = Insertbrzc(brid, brxm, brxb,
									brnl, sfzh, jtzz, sj, zcrq, ywckbm, ghybm);
						...
						// 当上次预约日期小于当前日期且上次挂号日期等于当天时执行
							...
							sqlString = sqlString
									+ Insertbrzc(brid, brxm, brxb, brnl, sfzh,
											jtzz, sj, zcrq, ywckbm, ghybm)
									+ "\t";
							sqlString = sqlString + updateyyghbrid(brid, yyid)
									+ "\t";
							sqlString = sqlString + " COMMIT TRAN";
					...
					// 当前预约时间等于上次预约日期时
						...
						sqlString = sqlString
								+ Insertyygh(yyid, ywckbm, ghybm, yyghrq, brxm,
										brxb, "", brnl, "1", sfzh, jtzz, sj,
										ysbm, yyks, yydjrq, yyyxrq, ghyks, ylkh)
								+ "\t";
	...
	public int getdatedifference(String date, Statement stmt) {
		...
		// Connection conn = JDBC.getConnection();
		// Statement stmt;
		String sql = "SELECT DATEDIFF(day , '" + tablename
				+ "',getdate()) as df ";
		try {
			// stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery(sql);
	...
	public boolean updateYwxhb2(String xh, String ssrq, Statement stmt) {
		boolean ok = false;
		String sql = " begin tran update ghb_ywxhb Set xh ='" + xh
				+ "' , ssrq ='" + ssrq + "' Where xhlx ='brid' commit tran ";
		try {
			ok = stmt.execute(sql);
			// stmt1.close(); // 关闭连接状态对象
			// conn1.commit();
		...
		try {
			...
			if (JdbcUtilSql.getuserghbrxx(sfzh, ylkh, stmt) == null) {
				...
				if (j >= 1) {
					...
					sqlString = sqlString
							+ Insertbrzc(brid, brxm, brxb, brnl, sfzh, jtzz,
									sj, zcrq, ywckbm, ghybm) + "" + "t";
					...
					if (k >= 1) {
						...
						sqlString = sqlString
								+ Insertghb(ghxh, ghzlbm, ghfsbm, brid, ghybm,
										yyghrq, yyks, ysbm, ghyks) + "\t";
						sqlString = sqlString + "commit tran" + "\t";
						System.out.print(sqlString);
						stmt.execute(sqlString);
					}
